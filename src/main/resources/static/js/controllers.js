...
angular.module('springChat.controllers', ['toaster'])
	.controller('ChatController', ['$scope', '$location', '$interval', 'toaster', 'ChatSocket', function($scope, $location, $interval, toaster, chatSocket) {
		...
		$scope.startTyping = function() {
	        ...
	        chatSocket.send("/topic/typing", {}, JSON.stringify({username: $scope.username, typing: true}));
		};
		...
		$scope.stopTyping = function() {
			if (angular.isDefined(typing)) {
		        ...
		        chatSocket.send("/topic/typing", {}, JSON.stringify({username: $scope.username, typing: false}));
			}
		};
		...
		var initStompClient = function() {
			...
			chatSocket.connect(function(frame) {
				...
				chatSocket.subscribe("/topic/typing", function(message) {
				...
				chatSocket.subscribe("/user/exchange/message", function(message) {
					var parsed = JSON.parse(message.body);
					parsed.priv = true;
					$scope.messages.unshift(parsed);
		        });
				  
				chatSocket.subscribe("/user/exchange/errors", function(message) {
